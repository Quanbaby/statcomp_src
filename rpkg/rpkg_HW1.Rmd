...
foobar <- function(x,z){
  s = check_input(x)
  if(s == 0){
    stop("The computation requires 1 observation(s) with value >= .001")
  }else{
    for (i in 1:s) {
      fit <- lm(x ~ z)
      r <- fit$residuals
...
```{r}
f <- function(x,b,a){
  return(colSums(x%*%t(b)) + rowSums(a))
}
```
...
`f` uses only about 20$\%$ time as that of `f0`.
...
library(matrixStats)
getT <- function(x, f){
  sp = (rowVars(x[,which(f==1)]) + rowVars(x[,which(f==2)]))*(little.n-1)/(n-2)
  return((rowMeans(x[,which(f==1)])-rowMeans(x[,which(f==2)]))/sqrt(sp/little.n*2))  
}
```
...
`getT` is about 1000 times faster than `getT0`.
